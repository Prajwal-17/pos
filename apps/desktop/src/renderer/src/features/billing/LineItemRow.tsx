  ...
  ({ idx, item, isCountColumnVisible, dragHandle, disableDrag }: LineItemRowProps) => {
    ...
    return (
      <div key={item.rowId} className="relative">
        <div
          className={`group ${checkedColor} border-border focus-within:border-border-strong grid min-h-(--billing-row-height) w-full items-center gap-1 rounded-[var(--radius-control)] border px-1 transition-[background-color,border-color] duration-150 ${
        ...
        >
            ...
            <div className="flex h-full items-center justify-between gap-1">
              {dragHandle && item.productSnapshot.trim() !== "" && !disableDrag ? (
                <GripVertical
                  ...
                  className="text-muted-foreground/60 hover:bg-accent hover:text-foreground active:bg-accent cursor-grab rounded-[var(--radius-control)] focus:outline-none active:cursor-grabbing"
                  size={20}
                />
              ) : (
                <GripVertical
                  className="text-muted-foreground/60 hover:bg-accent hover:text-foreground rounded-[var(--radius-control)] opacity-0 group-focus-within:opacity-100 group-hover:opacity-100 hover:cursor-grab focus:outline-none"
                  size={20}
                />
              )}
              ...
              <button
                ...
                className="text-destructive/75 hover:bg-destructive/10 hover:text-destructive flex size-7 items-center justify-center rounded-[var(--radius-control)] opacity-0 transition-[opacity,color,background-color] group-focus-within:opacity-100 group-hover:opacity-100 focus:opacity-100"
                onClick={() => {
          ...
          <div className="min-w-0">
            <input
              ...
              className="focus-visible:border-ring focus-visible:ring-ring/50 bg-background text-foreground placeholder:text-muted-foreground/80 border-border/80 h-9 w-full rounded-[var(--radius-control)] border px-3 py-2 text-sm font-medium shadow-none transition-[border-color,box-shadow,background-color,color] focus-visible:ring-2"
              onClick={(e) => {
          ...
          <div className="min-w-0">
            <div className="bg-muted/60 border-border/70 relative mx-auto flex h-9 w-full items-center rounded-[var(--radius-control)] border font-bold">
              <button
          ...
          <div className="min-w-0">
            <div className="relative h-9 w-full">
              ...
              <input
                ...
                className="focus-visible:border-ring focus-visible:ring-ring/50 bg-background text-foreground placeholder:text-muted-foreground/60 border-border/80 h-full w-full appearance-none rounded-[var(--radius-control)] border py-2 pr-3 pl-8 text-right text-sm font-semibold tabular-nums focus-visible:ring-2 disabled:cursor-not-allowed"
              />
            </div>
          </div>
          <div className="min-w-0">
            <div className="relative h-9 w-full">
              ...
              <div
                className={`bg-muted/40 border-border/70 text-foreground flex h-full w-full items-center justify-end rounded-[var(--radius-control)] border px-3 pl-8 text-right text-sm font-semibold tabular-nums transition-[background-color] duration-150 ${isFlash ? "bg-accent" : ""}`}
              >
          ...
          <div className="flex items-center justify-center">
            <button
              ...
              className={`flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center rounded-[var(--radius-control)] border transition-[border-color,box-shadow,background-color,color] ${
                checked
            ...
            <>
              ...
              <div className="flex min-w-0 items-center justify-center gap-1">
                <Button
                  ...
                  disabled={checked}
                  className="border-border/70 bg-muted/60 hover:bg-background flex h-8 w-8 cursor-pointer items-center justify-center rounded-[var(--radius-control)] p-0 shadow-none"
                >
