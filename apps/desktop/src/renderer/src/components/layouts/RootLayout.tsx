...
const LoadingState = ({ label = "Loading QuickCart…" }: { label?: string }) => (
  <div className="bg-background flex h-screen w-full items-center justify-center p-3">
    <div className="border-border bg-card flex min-w-56 flex-col items-center gap-3 rounded-[var(--radius-panel)] border p-4">
...
const RootLayout = () => {
  ...
  if (hasError) {
    return (
      <div className="bg-background flex h-screen w-full items-center justify-center p-3">
        <div className="border-border bg-card flex min-w-64 flex-col items-center gap-3 rounded-[var(--radius-panel)] border p-4 text-center">
          <div>
          ...
          <Button size="compact" onClick={() => queryClient.invalidateQueries()}>
            Retry
          </Button>
        </div>
      </div>
