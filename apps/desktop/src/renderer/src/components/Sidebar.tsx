...
export const Sidebar = ({ variant = "docked" }: SidebarProps) => {
  ...
  const shell = (
    <aside
      ...
      className={cn(
        "linear-light bg-sidebar text-sidebar-foreground relative h-full shrink-0 overflow-x-hidden overflow-y-auto border-r border-r-black/8",
        isOverlay ? "shadow-xl" : ""
      )}
      style={{
        width: sidebarWidth,
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="absolute bottom-0 left-0 w-full px-4 pb-4">
          <motion.div
            ...
            whileTap={{ scale: 0.97 }}
            transition={{ duration: 0.2, ease: [0.23, 1, 0.32, 1] }}
            className="border-border bg-secondary/80 hover:bg-secondary/90 flex cursor-pointer items-center gap-3 rounded-xl border p-3 backdrop-blur-md transition-colors duration-200"
          >
