...
import { getDailyEntry, listOnlineChannels, saveDailyEntry } from "@/features/money/money.repository";
  ...
  if (loading) {
    return (
      <SafeAreaView className="bg-canvas flex-1 items-center justify-center">
        <ActivityIndicator color="#B6532B" size="large" />
      ...
      >
        ...
        >
          <View className="w-full max-w-xl gap-3">
            <LedgerCard className="gap-3 p-3">
              <AmountInput
                label="Cash received"
