            ...
            {loading ? (
              ...
              names.map((name, index) => (
                <Pressable
                  ...
                  onPress={() => select(name)}
                  className={cn(
                    "min-h-12 justify-center px-3 py-3",
                    active === index && "bg-surface-muted"
                  )}
                  style={({ pressed }) => ({ backgroundColor: pressed ? "#E8EBE6" : undefined })}
                >
                  <Text className="text-ink text-base">{name}</Text>
                </Pressable>
              ))
            ) : (
